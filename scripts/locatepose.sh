...
# LocatePose：由 LocateAnything 的 lm_head 自回归生成框的两阶段训练。
#
# 用法：
#   bash scripts/locatepose.sh stage1
#   bash scripts/locatepose.sh stage2
...
# 本脚本默认使用物理卡 3；切换物理卡时显式设置 LOCATEPOSE_CUDA_VISIBLE_DEVICES。
export CUDA_VISIBLE_DEVICES="${LOCATEPOSE_CUDA_VISIBLE_DEVICES:-3}"
IFS=',' read -r -a VISIBLE_GPU_LIST <<< "${CUDA_VISIBLE_DEVICES}"
NPROC_PER_NODE="${#VISIBLE_GPU_LIST[@]}"
if (( NPROC_PER_NODE != 1 )); then
  echo "locatepose.sh 当前采用单进程安全配置；请只指定一张物理 GPU。" >&2
  exit 2
fi
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/locatepose/locatepose-locateanything-3b-${RUN_ID}}"
...
run_stage1() {
  ...
  "${PYTHON}" -m qwenpose.train_pose "${args[@]}"
}
...
echo "LocatePose 输出目录：${OUTPUT_DIR}"
echo "训练日志：${LOG_FILE}"
{
