...
#   1) 保留原 Stage1：冻结 Qwen 做 GT-box Pose warmup。
#   2) Teacher-forcing 预热：打开 Qwen LoRA，用 GT bbox JSON 做 LM 监督，PoseHead 吃 jitter GT box。
#   3) 闭环训练：Qwen 先 generate bbox JSON，解析后喂给 PoseHead，再用匹配到的 GT keypoints 监督。
#
...
#
# Stage 1 / FREEZE_QWEN GT-box warmup：
#   - 保持原一阶段语义：冻结 Qwen 主体与 LoRA，只训练 RGB 视觉分支、Qwen feature refiner 和 PoseHead。
#   - PoseHead 使用 GT box；默认训练 20 epoch；默认关闭 LM 辅助损失。
#
# Stage 2 / Teacher-forcing bbox warmup：
#   - 打开 Qwen LoRA，使用 GT bbox JSON 做 LM teacher-forcing 监督。
#   - PoseHead 使用 jitter GT box，提前适应非完美框；默认训练 3 epoch。
#
# Stage 3 / Closed-loop Qwen-box training：
#   - Qwen 先 generate bbox JSON，解析 bbox_2d 后作为 PoseHead 条件框。
#   - GT box 只用于匹配和监督，不再作为 PoseHead 输入；默认训练 12 epoch。
#
# 示例：
...
Options:
  --resume PATH   Resume from an existing run directory, stage directory, or
                  checkpoint path. The script auto-resolves whether to resume
                  stage 3 directly, initialize stage 3 from stage 2, or
                  initialize stage 2 from stage 1.
                  For explicit overrides, use STAGE*_RESUME_FROM_CHECKPOINT or
                  STAGE*_INIT_CHECKPOINT.
...
# CLI_RESUME_PATH：--resume 的专用别名，用于自动解析三阶段 run / stage / checkpoint 续训上下文。
...
STAGE2_NAMES = ("stage2_teacher_forcing", "stage2_qwen_lora_lm")
STAGE3_NAME = "stage3_qwen_box_closed_loop"
...
def stage_dirs(root: Path) -> tuple[Path, Path | None, Path]:
    s1 = root / STAGE1_NAME
...
def set_from_run(root: Path) -> None:
    global run_dir, stage1_dir, stage2_dir, stage3_dir
    global stage2_resume, stage3_resume, stage2_init, stage3_init
    global run_stage1, run_stage2, run_stage3
    run_dir = root
    s1, s2, s3 = stage_dirs(root)
    ...
    if s3.exists() and has_checkpoint_payload(s3):
        run_stage2 = "0"
        stage3_resume = str(s3)
    elif s2 is not None and has_checkpoint_payload(s2):
        run_stage2 = "0"
        stage3_init = str(s2)
...
if target.is_dir() and ((target / STAGE3_NAME).is_dir() or first_existing_stage(target, STAGE2_NAMES) is not None or (target / STAGE1_NAME).is_dir()):
    set_from_run(target)
elif target.is_dir() and target.name == STAGE3_NAME:
    run_dir = target.parent
    s1, s2, s3 = stage_dirs(run_dir)
    stage1_dir = str(s1) if s1.exists() else ""
    stage2_dir = str(s2) if s2 is not None else str(run_dir / STAGE2_NAMES[0])
    stage3_dir = str(target)
    stage3_resume = str(target)
    run_stage2 = "0"
elif target.parent.name == STAGE3_NAME:
    run_dir = target.parent.parent
    s1, s2, _ = stage_dirs(run_dir)
    stage1_dir = str(s1) if s1.exists() else ""
    stage2_dir = str(s2) if s2 is not None else str(run_dir / STAGE2_NAMES[0])
    ...
    run_dir = target.parent
    s1, _, s3 = stage_dirs(run_dir)
    stage1_dir = str(s1) if s1.exists() else ""
    stage2_dir = str(target)
    stage3_dir = str(s3)
    stage3_init = str(target)
    run_stage2 = "0"
...
if [[ -n "${CLI_RESUME_PATH}" ]]; then
  ...
  if [[ -n "${RESUME_RESOLVED_RUN_DIR:-}" ]]; then
    ...
    if [[ ! -v RUN_STAGE3 && -n "${RESUME_RESOLVED_RUN_STAGE3:-}" ]]; then
      RUN_STAGE3="${RESUME_RESOLVED_RUN_STAGE3}"
    fi
    ...
    if [[ ! -v STAGE3_OUTPUT_DIR && -n "${RESUME_RESOLVED_STAGE3_OUTPUT_DIR:-}" ]]; then
      STAGE3_OUTPUT_DIR="${RESUME_RESOLVED_STAGE3_OUTPUT_DIR}"
    fi
    ...
    if [[ ! -v STAGE3_INIT_CHECKPOINT && -n "${RESUME_RESOLVED_STAGE3_INIT_CHECKPOINT:-}" ]]; then
      STAGE3_INIT_CHECKPOINT="${RESUME_RESOLVED_STAGE3_INIT_CHECKPOINT}"
    fi
    if [[ ! -v STAGE3_RESUME_FROM_CHECKPOINT && -n "${RESUME_RESOLVED_STAGE3_RESUME:-}" ]]; then
      STAGE3_RESUME_FROM_CHECKPOINT="${RESUME_RESOLVED_STAGE3_RESUME}"
    fi
  fi
fi
...
# OUTPUT_ROOT：三阶段训练输出根目录。默认写入 outputs/qwenpose_three_stage_qwen。
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/qwenpose_three_stage_qwen}"

# RUN_NAME：本次 run 的目录名。默认由 RUN_NAME_BASE + RUN_TS 组成。
# 如果传入的 RUN_NAME 已经以 YYYYMMDD-HHMMSS 结尾，则不会重复追加时间戳。
RUN_NAME_BASE="${RUN_NAME:-qwenpose-three-stage-qwen3vl-lora}"
...
# OUTPUT_DIR：本次三阶段 run 的总目录。stage1/stage2/stage3 默认会放在这个目录下。
...
trap 'status=$?; echo "========== qwenpose three-stage train exit status ${status} at $(date -Is) =========="; exit ${status}' EXIT
...
# BATCH_SIZE：可选全局 per-GPU micro batch size。留空时 stage1 默认 4，stage2/stage3 默认 1。
...
# W_LM：全局 LM bbox 监督权重。stage1 默认强制为 0；stage2/stage3 可分别覆盖。
W_LM="${W_LM:-0.05}"
# LM_LOSS_EVERY：全局 LM loss 计算频率，单位为 micro batch。各 stage 可分别覆盖。
LM_LOSS_EVERY="${LM_LOSS_EVERY:-2}"
# LM_MAX_ANSWER_INSTANCES：ALL_POSE LM answer 中最多监督的人体实例数。默认跟 MAX_INSTANCES 对齐，避免 Qwen 只学前 10 个人。
...
# BOX_SOURCE：默认条件框来源。gt 使用标注框；qwen_generate 使用 Qwen 生成框闭环训练。
BOX_SOURCE="${BOX_SOURCE:-gt}"
# QWEN_BOX_MAX_NEW_TOKENS：闭环阶段 Qwen 生成 bbox JSON 的最大新 token 数。
QWEN_BOX_MAX_NEW_TOKENS="${QWEN_BOX_MAX_NEW_TOKENS:-4096}"
# BOX_MATCH_IOU_THRESH：闭环阶段 Qwen 生成框和 GT 框匹配的 IoU 阈值。
BOX_MATCH_IOU_THRESH="${BOX_MATCH_IOU_THRESH:-0.10}"
# BOX_NMS_IOU_THRESH：闭环阶段 Qwen 生成框 NMS 阈值。
...
# RUN_STAGE3：是否执行 stage3。1 执行；0 跳过。
...
STAGE2_OUTPUT_DIR="${STAGE2_OUTPUT_DIR:-${OUTPUT_DIR}/stage2_teacher_forcing}"
# STAGE3_OUTPUT_DIR：stage3 checkpoint、可视化和 batch trace 的输出目录。
STAGE3_OUTPUT_DIR="${STAGE3_OUTPUT_DIR:-${OUTPUT_DIR}/stage3_qwen_box_closed_loop}"
...
# STAGE3_INIT_WEIGHTS_DIR：自动生成的 stage3 weight-only init checkpoint 存放目录。
...
# STAGE3_TRAIN_DATASETS：stage3 训练数据集列表，逗号分隔。
STAGE3_TRAIN_DATASETS="${STAGE3_TRAIN_DATASETS:-${STAGE2_TRAIN_DATASETS}}"
...
# STAGE3_MIXING_STRATEGY：stage3 多数据集混合策略；默认继承 stage2。
STAGE3_MIXING_STRATEGY="${STAGE3_MIXING_STRATEGY:-${STAGE2_MIXING_STRATEGY}}"
...
# STAGE3_DATASET_MIX_WEIGHTS：stage3 数据集采样权重；默认继承 stage2。
STAGE3_DATASET_MIX_WEIGHTS="${STAGE3_DATASET_MIX_WEIGHTS:-${STAGE2_DATASET_MIX_WEIGHTS}}"
...
# STAGE3_MAX_SAMPLES_PER_DATASET：stage3 每个数据集最大样本数。空值表示全量。
STAGE3_MAX_SAMPLES_PER_DATASET="${STAGE3_MAX_SAMPLES_PER_DATASET:-${STAGE2_MAX_SAMPLES_PER_DATASET}}"
...
# STAGE3_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE：stage3 RefHuman 单实例最多 caption 数。默认继承 stage2。
STAGE3_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE="${STAGE3_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE:-${STAGE2_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE}}"
# STAGE1_EPOCHS：stage1 训练 epoch 数。默认 20，用于 RGB 视觉分支 pose warmup。
STAGE1_EPOCHS="${STAGE1_EPOCHS:-20}"
# STAGE2_EPOCHS：stage2 teacher-forcing 预热 epoch 数。默认 3。
STAGE2_EPOCHS="${STAGE2_EPOCHS:-3}"
# STAGE3_EPOCHS：stage3 闭环训练 epoch 数。默认 12。
STAGE3_EPOCHS="${STAGE3_EPOCHS:-12}"
...
# STAGE3_BATCH_SIZE：stage3 每张 GPU 的 micro batch size。默认 1。
STAGE3_BATCH_SIZE="${STAGE3_BATCH_SIZE:-${BATCH_SIZE:-1}}"
...
# STAGE3_GRAD_ACCUM_STEPS：stage3 梯度累积步数。默认 8。
STAGE3_GRAD_ACCUM_STEPS="${STAGE3_GRAD_ACCUM_STEPS:-8}"
...
# STAGE3_MAX_STEPS：stage3 最大 optimizer step 截断。0 表示按 12 epoch 完整训练。
...
# STAGE3_FREEZE_QWEN：stage3 是否冻结 Qwen 主体/LoRA。默认 0。
STAGE3_FREEZE_QWEN="${STAGE3_FREEZE_QWEN:-0}"
# STAGE1_W_LM：stage1 LM 辅助损失权重。默认 0。
STAGE1_W_LM="${STAGE1_W_LM:-0}"
# STAGE2_W_LM：stage2 teacher-forcing LM 辅助损失权重。默认 0.5。
STAGE2_W_LM="${STAGE2_W_LM:-0.5}"
# STAGE3_W_LM：stage3 闭环阶段 LM 辅助损失权重。默认 0.2。
STAGE3_W_LM="${STAGE3_W_LM:-0.2}"
# STAGE1_LM_LOSS_EVERY：stage1 LM loss 计算频率。默认 0。
STAGE1_LM_LOSS_EVERY="${STAGE1_LM_LOSS_EVERY:-0}"
# STAGE2_LM_LOSS_EVERY：stage2 LM loss 计算频率。默认每个 micro batch 计算。
STAGE2_LM_LOSS_EVERY="${STAGE2_LM_LOSS_EVERY:-1}"
# STAGE3_LM_LOSS_EVERY：stage3 LM loss 计算频率。默认每个 micro batch 计算。
STAGE3_LM_LOSS_EVERY="${STAGE3_LM_LOSS_EVERY:-1}"
# STAGE1_BOX_SOURCE：stage1 PoseHead 条件框来源；保持原始 GT box。
STAGE1_BOX_SOURCE="${STAGE1_BOX_SOURCE:-gt}"
# STAGE2_BOX_SOURCE：stage2 PoseHead 条件框来源；teacher-forcing 使用 GT box。
STAGE2_BOX_SOURCE="${STAGE2_BOX_SOURCE:-gt}"
# STAGE3_BOX_SOURCE：stage3 PoseHead 条件框来源；闭环使用 Qwen 生成框。
STAGE3_BOX_SOURCE="${STAGE3_BOX_SOURCE:-qwen_generate}"
...
# STAGE2_BOX_JITTER_SCALE：stage2 teacher-forcing 条件框缩放扰动。
STAGE2_BOX_JITTER_SCALE="${STAGE2_BOX_JITTER_SCALE:-0.08}"
# STAGE2_BOX_JITTER_SHIFT：stage2 teacher-forcing 条件框中心平移扰动。
STAGE2_BOX_JITTER_SHIFT="${STAGE2_BOX_JITTER_SHIFT:-0.04}"
# STAGE3_BOX_JITTER_SCALE：stage3 生成框不再叠加 jitter。
STAGE3_BOX_JITTER_SCALE="${STAGE3_BOX_JITTER_SCALE:-0.0}"
# STAGE3_BOX_JITTER_SHIFT：stage3 生成框不再叠加 jitter。
STAGE3_BOX_JITTER_SHIFT="${STAGE3_BOX_JITTER_SHIFT:-0.0}"
# STAGE1_QWEN_BOX_MAX_NEW_TOKENS：stage1 不使用，仅占位保持 run_stage 参数一致。
...
STAGE3_QWEN_BOX_MAX_NEW_TOKENS="${STAGE3_QWEN_BOX_MAX_NEW_TOKENS:-${QWEN_BOX_MAX_NEW_TOKENS}}"
...
STAGE3_BOX_MATCH_IOU_THRESH="${STAGE3_BOX_MATCH_IOU_THRESH:-${BOX_MATCH_IOU_THRESH}}"
...
STAGE3_BOX_NMS_IOU_THRESH="${STAGE3_BOX_NMS_IOU_THRESH:-${BOX_NMS_IOU_THRESH}}"
# STAGE1_LR：stage1 基础学习率。默认继承 LR。
STAGE1_LR="${STAGE1_LR:-${LR}}"
# STAGE2_LR：stage2 基础学习率。默认 1e-4。
STAGE2_LR="${STAGE2_LR:-1e-4}"
# STAGE3_LR：stage3 基础学习率。默认 5e-5，闭环阶段更稳。
...
# STAGE3_QWEN_LORA_LR_SCALE：stage3 LLM LoRA lr scale。
STAGE3_QWEN_LORA_LR_SCALE="${STAGE3_QWEN_LORA_LR_SCALE:-${QWEN_LORA_LR_SCALE}}"
...
# STAGE3_QWEN_VISION_LR_SCALE：stage3 vision LoRA lr scale。
STAGE3_QWEN_VISION_LR_SCALE="${STAGE3_QWEN_VISION_LR_SCALE:-${QWEN_VISION_LR_SCALE}}"
...
# STAGE3_WARMUP_STEPS：stage3 warmup step 数。
STAGE3_WARMUP_STEPS="${STAGE3_WARMUP_STEPS:-${WARMUP_STEPS}}"
...
# STAGE3_MIN_LR_RATIO：stage3 cosine scheduler 最低学习率比例。
STAGE3_MIN_LR_RATIO="${STAGE3_MIN_LR_RATIO:-${MIN_LR_RATIO}}"
...
# STAGE3_NUM_WORKERS：stage3 DataLoader worker 数。
STAGE3_NUM_WORKERS="${STAGE3_NUM_WORKERS:-${NUM_WORKERS}}"
...
# STAGE3_PREFETCH_FACTOR：stage3 DataLoader prefetch factor。
STAGE3_PREFETCH_FACTOR="${STAGE3_PREFETCH_FACTOR:-${PREFETCH_FACTOR}}"
...
# STAGE3_SAVE_EVERY：stage3 checkpoint 保存间隔。
STAGE3_SAVE_EVERY="${STAGE3_SAVE_EVERY:-${SAVE_EVERY}}"
...
# STAGE3_SAVE_TOTAL_LIMIT：stage3 最多保留 checkpoint 数量。
STAGE3_SAVE_TOTAL_LIMIT="${STAGE3_SAVE_TOTAL_LIMIT:-${SAVE_TOTAL_LIMIT}}"
...
# STAGE3_VISUALIZE_EVERY：stage3 可视化保存间隔。
STAGE3_VISUALIZE_EVERY="${STAGE3_VISUALIZE_EVERY:-${VISUALIZE_EVERY}}"
...
# STAGE3_DISABLE_BATCH_TRACE：stage3 是否关闭每 batch JSONL trace。默认继承全局开关。
STAGE3_DISABLE_BATCH_TRACE="${STAGE3_DISABLE_BATCH_TRACE:-${DISABLE_BATCH_TRACE}}"
# STAGE1_SEED：stage1 随机种子。
STAGE1_SEED="${STAGE1_SEED:-${SEED}}"
# STAGE2_SEED：stage2 随机种子。
STAGE2_SEED="${STAGE2_SEED:-${SEED}}"
# STAGE3_SEED：stage3 随机种子。
STAGE3_SEED="${STAGE3_SEED:-${SEED}}"
...
# STAGE3_RESUME_FROM_CHECKPOINT：stage3 真实断点续训路径。none 表示不续训。
STAGE3_RESUME_FROM_CHECKPOINT="${STAGE3_RESUME_FROM_CHECKPOINT:-none}"
# STAGE2_INIT_CHECKPOINT：stage2 显式 weight-only 初始化来源，只加载模型权重，不恢复训练游标。
STAGE2_INIT_CHECKPOINT="${STAGE2_INIT_CHECKPOINT:-}"
# STAGE3_INIT_CHECKPOINT：stage3 显式 weight-only 初始化来源，只加载模型权重，不恢复训练游标。
...
# STAGE3_INIT_FROM_STAGE2：stage3 是否默认从 stage2 输出生成 weight-only init。1 表示是。
STAGE3_INIT_FROM_STAGE2="${STAGE3_INIT_FROM_STAGE2:-1}"
# MERGE_FINAL_WEIGHTS：stage3 正常结束后是否自动合并 LoRA 为完整发布权重。
...
if [[ -n "${CLI_RESUME_PATH}" ]]; then
  ...
  if [[ "${STAGE3_RESUME_FROM_CHECKPOINT}" == "none" && -n "${RESUME_RESOLVED_STAGE3_RESUME:-}" ]]; then
    STAGE3_RESUME_FROM_CHECKPOINT="${RESUME_RESOLVED_STAGE3_RESUME}"
  fi
  if [[ -z "${STAGE3_INIT_CHECKPOINT}" && -n "${RESUME_RESOLVED_STAGE3_INIT_CHECKPOINT:-}" ]]; then
    STAGE3_INIT_CHECKPOINT="${RESUME_RESOLVED_STAGE3_INIT_CHECKPOINT}"
  fi
fi
...
require_bool STAGE3_DISABLE_BATCH_TRACE "${STAGE3_DISABLE_BATCH_TRACE}"
...
for spec in \
  ...
  "STAGE2_QWEN_BOX_MAX_NEW_TOKENS:${STAGE2_QWEN_BOX_MAX_NEW_TOKENS}" \
  "STAGE3_QWEN_BOX_MAX_NEW_TOKENS:${STAGE3_QWEN_BOX_MAX_NEW_TOKENS}"; do
...
for spec in \
  ...
  "STAGE3_VISUALIZE_EVERY:${STAGE3_VISUALIZE_EVERY}" \
  "STAGE1_LM_LOSS_EVERY:${STAGE1_LM_LOSS_EVERY}" \
  "STAGE2_LM_LOSS_EVERY:${STAGE2_LM_LOSS_EVERY}" \
  "STAGE3_LM_LOSS_EVERY:${STAGE3_LM_LOSS_EVERY}" \
  "STAGE1_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE:${STAGE1_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE}" \
  "STAGE2_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE:${STAGE2_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE}" \
  "STAGE3_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE:${STAGE3_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE}"; do
...
if [[ "${RUN_STAGE3}" == "1" && "${STAGE3_BOX_SOURCE}" == "qwen_generate" && "${ZERO_STAGE}" != "zero2" && "${ZERO_STAGE}" != "none" ]]; then
  echo "Stage 3 qwen_generate calls model.generate during training and currently supports ZERO_STAGE=zero2 or none. Got ZERO_STAGE=${ZERO_STAGE}." >&2
  exit 1
fi
...
check_optional_checkpoint STAGE3_RESUME_FROM_CHECKPOINT "${STAGE3_RESUME_FROM_CHECKPOINT}"
check_optional_checkpoint STAGE2_INIT_CHECKPOINT "${STAGE2_INIT_CHECKPOINT}"
check_optional_checkpoint STAGE3_INIT_CHECKPOINT "${STAGE3_INIT_CHECKPOINT}"
...
run_stage() {
  ...
  local box_source="${28}"
  local box_jitter_scale="${29}"
  local box_jitter_shift="${30}"
  local qwen_box_max_new_tokens="${31}"
  local box_match_iou_thresh="${32}"
  local box_nms_iou_thresh="${33}"
...
echo "STAGE3_INIT_FROM_STAGE2=${STAGE3_INIT_FROM_STAGE2}"
echo "STAGE2_INIT_CHECKPOINT=${STAGE2_INIT_CHECKPOINT}"
echo "STAGE3_INIT_CHECKPOINT=${STAGE3_INIT_CHECKPOINT}"
echo "STAGE2_RESUME_FROM_CHECKPOINT=${STAGE2_RESUME_FROM_CHECKPOINT}"
echo "STAGE3_RESUME_FROM_CHECKPOINT=${STAGE3_RESUME_FROM_CHECKPOINT}"
...
if [[ "${RUN_STAGE1}" == "1" ]]; then
  run_stage \
    ...
    "${STAGE1_QWEN_BOX_MAX_NEW_TOKENS}" \
    "${STAGE1_BOX_MATCH_IOU_THRESH}" \
    "${STAGE1_BOX_NMS_IOU_THRESH}"
else
...
if [[ "${RUN_STAGE2}" == "1" ]]; then
  ...
  run_stage \
    ...
    "${STAGE2_QWEN_BOX_MAX_NEW_TOKENS}" \
    "${STAGE2_BOX_MATCH_IOU_THRESH}" \
    "${STAGE2_BOX_NMS_IOU_THRESH}"
else
  echo "Skipping stage 2 because RUN_STAGE2=0"
fi

if [[ "${RUN_STAGE3}" == "1" ]]; then
  stage3_resume_arg="${STAGE3_RESUME_FROM_CHECKPOINT}"
  if [[ "${stage3_resume_arg}" == "none" || -z "${stage3_resume_arg}" ]]; then
    if [[ "${DRY_RUN_DATA}" == "1" ]]; then
      stage3_resume_arg="none"
    elif [[ -n "${STAGE3_INIT_CHECKPOINT}" ]]; then
      echo "Preparing stage 3 weight-only init from STAGE3_INIT_CHECKPOINT=${STAGE3_INIT_CHECKPOINT}"
      stage3_resume_arg="$(prepare_weights_only_checkpoint "${STAGE3_INIT_CHECKPOINT}" "${STAGE3_INIT_WEIGHTS_DIR}")"
    elif [[ "${STAGE3_INIT_FROM_STAGE2}" == "1" ]]; then
      echo "Preparing stage 3 weight-only init from stage 2 output: ${STAGE2_OUTPUT_DIR}"
      stage3_resume_arg="$(prepare_weights_only_checkpoint "${STAGE2_OUTPUT_DIR}" "${STAGE3_INIT_WEIGHTS_DIR}")"
    else
      stage3_resume_arg="none"
      echo "Stage 3 will start from base Qwen + newly initialized pose modules because STAGE3_INIT_FROM_STAGE2=0."
    fi
  else
    echo "Stage 3 will resume checkpoint state from ${stage3_resume_arg}"
  fi

  run_stage \
    "Stage 3 / Closed-loop Qwen-box training" \
    "${STAGE3_OUTPUT_DIR}" \
    "${STAGE3_TRAIN_DATASETS}" \
    "${STAGE3_BATCH_SIZE}" \
    "${STAGE3_GRAD_ACCUM_STEPS}" \
    "${STAGE3_EPOCHS}" \
    "${STAGE3_MAX_STEPS}" \
    "${STAGE3_FREEZE_QWEN}" \
    "${STAGE3_W_LM}" \
    "${STAGE3_LM_LOSS_EVERY}" \
    "${STAGE3_REFHUMAN_MAX_CAPTIONS_PER_INSTANCE}" \
    "${STAGE3_MIXING_STRATEGY}" \
    "${STAGE3_DATASET_MIX_WEIGHTS}" \
    "${STAGE3_MAX_SAMPLES_PER_DATASET}" \
    "${STAGE3_LR}" \
    "${STAGE3_QWEN_LORA_LR_SCALE}" \
    "${STAGE3_QWEN_VISION_LR_SCALE}" \
    "${STAGE3_WARMUP_STEPS}" \
    "${STAGE3_MIN_LR_RATIO}" \
    "${STAGE3_NUM_WORKERS}" \
    "${STAGE3_PREFETCH_FACTOR}" \
    "${STAGE3_SAVE_EVERY}" \
    "${STAGE3_SAVE_TOTAL_LIMIT}" \
    "${STAGE3_VISUALIZE_EVERY}" \
    "${STAGE3_SEED}" \
    "${stage3_resume_arg}" \
    "${STAGE3_DISABLE_BATCH_TRACE}" \
    "${STAGE3_BOX_SOURCE}" \
    "${STAGE3_BOX_JITTER_SCALE}" \
    "${STAGE3_BOX_JITTER_SHIFT}" \
    "${STAGE3_QWEN_BOX_MAX_NEW_TOKENS}" \
    "${STAGE3_BOX_MATCH_IOU_THRESH}" \
    "${STAGE3_BOX_NMS_IOU_THRESH}"

  if [[ "${DRY_RUN_DATA}" != "1" && "${MERGE_FINAL_WEIGHTS}" == "1" ]]; then
    merge_full_weights "${STAGE3_OUTPUT_DIR}" "${MERGED_WEIGHTS_DIR}"
  fi
else
  echo "Skipping stage 3 because RUN_STAGE3=0"
fi
